...
//        barberImage.kf.setImage(with: url)
    ...
    @objc func buttonAction(){
        ...
        let string = self.time   // "March 24, 2017 at 7:00 AM" '20 February 2018 12:00'
        let dateComp = string.components(separatedBy: ":")
       let Later = dataToPost.sharedManager.duration
        
        var endHr = Int(dateComp[0])! +  (Later/60)
        var endMin = Int(dateComp[1])! +  (Later%60)
        
        let endTime = String(endHr) + ":" + String(endMin)
...
//        let endTime = calendar.addingTimeInterval(Later)
...
//                dataToPost.sharedManager.endDate = endTime
...
//        print("endDate \(endTime)")
        ...
        let spin2 = UIViewController.displaySpinner(onView: self.view)
            ref = db.collection("bookings").addDocument(data: [
                ...
                if let err = err {
                    ...
                    dataToPost.sharedManager.date = ""
                    dataToPost.sharedManager.imgUrl = ""
                    dataToPost.sharedManager.duration = 0
                    dataToPost.sharedManager.startTime = ""
                    dataToPost.sharedManager.endTime = ""
    ...
    @objc func doneClicked(){
        ...
        dateFormattr.locale = Locale(identifier: "en_US_POSIX")
        dateFormattr.dateFormat = "yyyy-MM-dd"
        
        let strings = self.date
        let event_date = dateFormattr.date(from: strings)
        
        let datFormatter1 = DateFormatter()
        
        datFormatter1.dateFormat = "MMMM dd, yyyy "
        datFormatter1.locale = Locale(identifier: "en_US")
        let datesel = datFormatter1.string(from: event_date!)
        
        
        datePickButton.setTitle(datesel, for: .normal)
        ...
        let parameters :  [String : Any] = ["date" : self.date , "currentStylistID" : dataToPost.sharedManager.stylistId ]
