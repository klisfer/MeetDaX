...
class OTPViewController: UIViewController {
    ...
    private func setupLayout(){
        ...
        resendOtp.frame = CGRect(x: screenWidth - (0.65*screenWidth), y: 400 , width: screenWidth-40, height: 50)
        //resendOtp.backgroundColor = UIColor(hexString: "#1C92AFff")
        //resendOtp.layer.shadowColor = UIColor.black.cgColor
        //resendOtp.layer.shadowOffset = CGSize(width: 2, height: 2)
        //resendOtp.layer.shadowRadius = 2
        //resendOtp.layer.shadowOpacity = 1.0
        resendOtp.setTitle("Resend OTP", for: UIControlState.normal)
        resendOtp.setTitleColor(UIColor.white, for: .normal)
        resendOtp.tintColor = UIColor.black
        
        //resendOtp.layer.cornerRadius = 6
        resendOtp.addTarget(self, action: #selector(resendOtpAct),for: .touchUpInside)
        view.addSubview(resendOtp)
        
        
    }
    
    @objc private func resendOtpAct(sender: Any){
        
        PhoneAuthProvider.provider().verifyPhoneNumber(customerDetails.sharedManager1.mobileNumber , uiDelegate: nil) { (verificationID, error) in
            if let error = error {
